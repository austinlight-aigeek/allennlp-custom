...
from srl_bert.common.util import sanitize_wordpiece
...
class PretrainedTransformerTokenizer(Tokenizer):
    ...
    def _reverse_engineer_special_tokens(
        ...
        for token_id, token_type_id in zip(dummy_output["input_ids"], dummy_output["token_type_ids"]):
            if token_id == dummy_a:
                ...
                seen_dummy_a = True
                assert (
                    self.single_sequence_token_type_id is None or self.single_sequence_token_type_id == token_type_id
    ...
    def tokenizer_lowercases(tokenizer: PreTrainedTokenizer) -> bool:
        ...
        tokenized = tokenizer.tokenize("A")  # Use a single character that won't be cut into word pieces.
