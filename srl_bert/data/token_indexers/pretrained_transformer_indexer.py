...
import logging
import torch
from srl_bert.common.util import pad_sequence_to_length


from srl_bert.data.vocabulary import Vocabulary
from srl_bert.data.tokenizers import Token, PretrainedTransformerTokenizer
from srl_bert.data.token_indexers.token_indexer import TokenIndexer, IndexedTokenList
...
class PretrainedTransformerIndexer(TokenIndexer):
    ...
    def _extract_token_and_type_ids(self, tokens: List[Token]) -> Tuple[List[int], List[int]]:
        ...
        for token in tokens:
            indices.append(
                token.text_id if token.text_id is not None else self._tokenizer.convert_tokens_to_ids(token.text)
            )
    ...
    def _postprocess_output(self, output: IndexedTokenList) -> IndexedTokenList:
        ...
        if self._max_length is not None:
            ...
            # Folds indices
            folded_indices = [
                indices[i : i + self._effective_max_length] for i in range(0, len(indices), self._effective_max_length)
            ]
            folded_type_ids = [
            ...
            folded_type_ids = [
                single_sequence_start_type_ids + segment + single_sequence_end_type_ids for segment in folded_type_ids
            ]
            assert all(
