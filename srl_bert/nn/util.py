...
from srl_bert.common.util import int_to_device, is_distributed, is_global_primary
...
def get_text_field_mask(
    ...
    tensor_dims = [
        (tensor.dim(), tensor) for indexer_output in text_field_tensors.values() for tensor in indexer_output.values()
    ]
...
def find_text_field_embedder(model: torch.nn.Module) -> torch.nn.Module:
    ...
    """
    from srl_bert.modules.text_field_embedders.text_field_embedder import TextFieldEmbedder
...
def find_embedding_layer(model: torch.nn.Module) -> torch.nn.Module:
    ...
    from srl_bert.modules.text_field_embedders.text_field_embedder import TextFieldEmbedder
    from srl_bert.modules.text_field_embedders.basic_text_field_embedder import (
...
def load_state_dict_distributed(
    ...
    from srl_bert.nn.parallel.sharded_module_mixin import ShardedModuleMixin
