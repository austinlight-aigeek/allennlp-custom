...
CACHE_ROOT = Path(os.getenv("ALLENNLP_CACHE_ROOT", Path.home() / ".srl_bert"))
...
class LocalCacheResource:
    ...
    def writer(self, mode="w"):
        if self.cached():
            raise ValueError(f"local cache of {self.resource_name} (version '{self.version}') already exists!")
    ...
    def reader(self, mode="r"):
        if not self.cached():
            raise ValueError(f"local cache of {self.resource_name} (version '{self.version}') does not exist yet!")
...
def inspect_cache(patterns: List[str] = None, cache_dir: Union[str, Path] = None):
    ...
    """
    from srl_bert.common.util import format_timedelta, format_size
